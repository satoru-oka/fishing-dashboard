...
- **E. 各パネルの実データ仕様** … **データ契約として定義済み** → `docs/deep-dive-data-contract.md`。各パネル（KPI＋②〜⑥＋§5 表）のサーバ集計エンドポイント I/O を、期間・モード・スポット可変＋メタ（n/充足/最終更新）で規定。catch-management `#72` の実装契約になる。
