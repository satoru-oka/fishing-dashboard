...
> このドキュメントは、釣果管理アプリ「現場グランス」画面に表示する指標の定義と、その設計判断の根拠を記録したもの。後から読み返して「なぜこのロジックなのか」が分かることを目的とする。統合・将来拡張に関する議論は fishing-dashboard #40 を参照。
...
- `sessions.tide_phase` enum — 集計可能な潮回り（大潮/中潮/小潮/長潮/若潮）。自由記述の `tide` とは別に持つ。
- `sessions.started_at` — **必須**。ヒット率の分母・時間窓・ボウズ記録の土台。
- `catches.length_cm` — ベスト記録用。
- タックル参照 — `lures` テーブル（淡水系）または `catches.rig`（海系）。

> SQL は概略。テーブル名・カラム名（`caught_at` / `lure_id` 等）は実スキーマに合わせて調整する。スコープの源は `:spot_id`＝GPS で特定した現在地、`:tide`＝潮汐 API で算出した現在の潮回り。ユーザー絞り込みは Supabase RLS に任せる。
...
- 限界: 尾数最多は「よく行く時間帯」が出るだけの努力バイアスを含む。信頼版は時間帯別ヒット率（`started_at` を分母にした努力ログが必要）。
- 最低サンプル: 総釣果 < 10 → 「収集中」
...
- 定義: このスポット・指定潮回りで、1尾以上釣れた釣行の割合（直近12ヶ月）。分母は釣行（session）。
...
where s.spot_id    = :spot_id
  ...
  and s.started_at >= now() - interval '12 months';
```
